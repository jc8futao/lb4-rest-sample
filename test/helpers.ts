...
export async function givenAConnectedDataSource(): Promise<RestSampleDataSource> {
  ...
  config.operations[0].template.url = `http://${restConfig.host}:${restConfig.port}/test/rest/{id}`
...
export async function setupApplication(): Promise<AppWithClient> {
  ...
  app.route('GET', '/test/rest/', { responses: { '200': PING_RESPONSE } }, async () => Promise.resolve(restSampleResponse));
  app.route('GET', '/test/rest/1', { responses: { '200': PING_RESPONSE, } }, async () => Promise.resolve(restSampleResponse[0]));
